...
if (!isset($_GET["id"])) {
    header("Location: ver_padres.php");
    exit;
}

$id = $_GET["id"];

$stmt = $conexion->prepare("SELECT id, nombre, email FROM usuarios WHERE id = :id AND rol = 'padre'");
$stmt->execute([":id" => $id]);
    ...
    if ($nombre === "" || $email === "") {
        ...
        try {
            ...
            if ($ver->fetch()) {
                ...
                $_SESSION["success_msg"] = "Padre actualizado correctamente.";
                header("Location: ver_padres.php");
                exit;
            }
        } catch (PDOException $e) {
            $mensaje = "Error al actualizar el padre.";
            $tipo = "danger";
        }
    }
}

$pageTitle = "Editar Padre";
    ...
    <main id="main">
        <div class="topbar">
            ...
            <div class="breadcrumb-bar">
                <h5>Editar Padre</h5>
                <p>Panel Administrativo &nbsp;/&nbsp; Editar Padre</p>
            </div>
        </div>
                        ...
                        <form method="POST" autocomplete="off">
                            <div class="mb-3">
                                ...
                                <input type="text" name="nombre" class="form-control"
                                    value="<?= htmlspecialchars($_POST["nombre"] ?? $padre["nombre"]) ?>" required
                                    placeholder="Ej. Ricardo Gómez">
                            </div>

                            <div class="mb-3">
                                <label class="form-label fw-medium">Correo electrónico</label>
                                <input type="email" name="email" class="form-control" placeholder="padre@gca.edu.co"
                                    value="<?= htmlspecialchars($_POST["email"] ?? $padre["email"]) ?>" required>
                            </div>
                            ...
                            <a href="ver_padres.php" class="btn-outline-gca w-100 justify-content-center mt-2">
